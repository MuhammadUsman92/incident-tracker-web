...
import { criminalMapLocationGet } from '../actions/criminalMapLocationActions';
...
const CriminalMapScreen = () => {
  ...
  const getCriminalMapLocation = useSelector((state) => state.getCriminalMapLocation);
  const { loading, response, error } = getCriminalMapLocation;
  ...
  const getPatientBtnHandler = () => {
    ...
    dispatch(criminalMapLocationGet(navigate, formData));
  };

  const data = response?.data;
  const crimesCount = data?.length;

  // Count crime types
  const crimeTypesCount = data?.reduce((count, crime) => {
    const { type } = crime;
    count[type] = (count[type] || 0) + 1;
    return count;
  }, {});

  const chartData = crimeTypesCount
    ? Object.keys(crimeTypesCount).map((type) => ({
        name: type,
        count: crimeTypesCount[type],
      }))
    : null;

  const pieChartData = crimeTypesCount
    ? Object.keys(crimeTypesCount).map((type) => ({
        name: type,
        value: crimeTypesCount[type],
      }))
    : null;
  ...
  const addMarkers = (locations) => {
    ...
    const createdMarkers = locations.map((crime) => {
      const { id, incidentLocation, type } = crime;
      const { street, city, postal_code, latitude, longitude, country } = incidentLocation;
    ...
    <>
      ...
      <Row className="mt-3">
        ...
        <Col className="text-center">
          <button className="btn btn-primary" disabled={loading} onClick={getPatientBtnHandler}>
